...
class MaterielsTravauxPratiquesController extends AppController
{
    ...
    public function view(){
        ...
        //count the nb of machine for each TP
        foreach ($listMateriels as $key => $materiel) {
            $nbTPs = 0;
            $color = 'info';
    ...
    //we get all TP name for each Matériel
    protected function listTPs($listMateriels){
        foreach ($listMateriels as $materiel) {
            //debug($materiel->travaux_pratiques);
            $TPlist = $materiel->travaux_pratiques;
            $list = '';

            foreach ($TPlist as $TP) {  //concat all TP names line and add a EOL
                $list = $list.$TP->fullName."</br>";
                //debug($list);
            }

            $materiel->TPlist = $list;
        }
    }
