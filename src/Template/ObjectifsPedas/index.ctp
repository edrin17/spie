<?php 
    $this->assign('title', 'Liste des Objectifs Pédagogiques');
    $this->set('modalTitle','Ajouter une nouvel objectif');
...
<div class="row">
    <div class="col-lg-12">       
        ...
        <div class="col-lg-1 col-lg">
            <?php echo $this->element('/Modals/NewEntryObj'); ?>
        </div>
		...
		<div class="col-lg-10 ">
            <?php echo $this->Form->input('competences_intermediaire_id', [
                'label' => 'Filtrer par compétence intermédiare:',
                'onchange' => 'filtreObjectifsByCompInter()',
                'options' => $competencesIntermediaires,
                'default' => $competences_intermediaire_id
            ]); ?>
        </div>
        <table class="table">
            <thead>
            ...
            <tbody>
                <?php foreach ($objectifs as $objectif): ?> <!--Affiche le contenu de 'capacitess'  -->
                <tr> 
                    <td><?= h($objectif->fullName) ?></td> <!-- Ici on ajoute C. pour avoir une compétence de la forme C.3.2.1 -->
                    <td class="actions">
                    <!-- Affiche des urls/boutons et de leurs actions -->
                    <!-- Modal edit -->
                    <?php $this->set('competencesIntermediaire',$objectif); ?>
                    ...
                    <?php echo $this->element('/Modals/EditObj'); ?>
                    <!-- /Modal edit -->
                    <!-- Button delete -->
                    <?php $this->set('object',$objectif); ?>
...
<script type="text/javascript">
	...
	function filtreObjectifsByCompInter() {
		...
		var competences_intermediaire_id = document.getElementById("competences-intermediaire-id").value;
        var url = "<?php echo $this->Url->build([
            'controller'=>'ObjectifsPedas','action'=>'index']) ?>" 
            + "?referential_id=" + referential_id + "&capacite_id="
            + capacite_id + "&competences_terminale_id=" + competences_terminale_id + "&competences_intermediaire_id=" + competences_intermediaire_id
        window.location = url;
    }
	...
	function filtreCompetencesInterByCompTerm() {
        ...
        var url = "<?php echo $this->Url->build([
                ...
                'action' => 'chainedCompetencesIntermediaires'
            ]); ?>/?referential_id=" + referential_id + "&capacite_id=" + capacite_id + "&competences_terminale_id=" + competences_terminale_id;
        ...
        $('#competences-intermediaire-id').html(response);

        // Call the next function in the sequence
        filtreObjectifsByCompInter();
    }
